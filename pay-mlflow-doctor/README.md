...
8. `.env`, `config.yaml`, `Dockerfile`: password, username, token, API key 등 credential-like 값
9. `pyproject.toml`: requires-python, dependency 기준
10. artifact path: MLmodel 내부 참조 경로 실제 존재 여부
11. uri: `models:/`, `runs:/`, `file:`, `s3://`, `gs://`, Windows path
