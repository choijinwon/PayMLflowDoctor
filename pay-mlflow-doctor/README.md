...
8. `pyproject.toml`: requires-python, dependency 기준
9. artifact path: MLmodel 내부 참조 경로 실제 존재 여부
10. uri: `models:/`, `runs:/`, `file:`, `s3://`, `gs://`, Windows path
...
```bash
paymlflow-doctor validate sample-data/broken-mlflow-project --format markdown
```

JSON 리포트:

```bash
paymlflow-doctor validate sample-data/broken-mlflow-project --format json --output logs/demo-report.json
```

안전 수정 계획:

```bash
paymlflow-doctor fix sample-data/broken-mlflow-project
```
...
5. 실제 도입 시 보안상 안전한가?
   secret은 읽더라도 출력하지 않는 정책으로 확장할 수 있고, 현재 MVP는 placeholder와 정책 제안 중심입니다. production 값은 자동 생성하지 않습니다.
